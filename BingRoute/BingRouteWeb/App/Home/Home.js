/// <reference path="../App.js" />
    ...
    function RegisterCallbacks(binding) {
        ...
        Office.select("bindings#" + BindingName).getDataAsync({
            ...
            if (callback.status == Office.AsyncResultStatus.Succeeded) {
                PostField = callback.value.headers[0][1];
            }
        });
    }
    ...
    function UpdateMap() {
        Office.select("bindings#" + BindingName).getDataAsync({
            ...
            if (callback.status == Office.AsyncResultStatus.Succeeded) {
                ToAddress = callback.value.rows[0][0];
                ClickRoute();
            }
        });
    }
    ...
    function MakeRouteRequest(credentials) {
        var routeRequest = "https://dev.virtualearth.net/REST/v1/Routes?wp.0=14012 110th PL NE, Kirkland, WA&wp.1=" + ToAddress + "&routePathOutput=Points&output=json&jsonp=RouteCallback&key=" + credentials;
      ...
      function RouteCallback(result) {
        ...
        if (result &&
            ...
            var milesTraveled = result.resourceSets[0].resources[0].travelDistance * 0.62137119;
