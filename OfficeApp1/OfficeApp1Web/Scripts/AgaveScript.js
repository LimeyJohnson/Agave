...
OfficeApp1Script.AgaveScript.setInitialData=function(eventArgs){var $0=new Office.TableData();$0.rows=[['Andrew','Johnson'],['John','Morrison']];$0.headers=[['FirstName','LastName']];var $1={};$1.coercionType=Office.CoercionType.Table;Office.context.document.setSelectedDataAsync($0,$1,function($p1_0){
if($p1_0.status===Office.AsyncResultStatus.Failed){write($p1_0.error.name + ' : '+$p1_0.error.message);}var $1_0={};$1_0.id=OfficeApp1Script.AgaveScript.tableBinding;Office.context.document.bindings.addFromSelectionAsync(Office.BindingType.Table,$1_0,function($p2_0){
});});}
OfficeApp1Script.AgaveScript.setSecondData=function(eventArgs){var $0=new Office.TableData();$0.rows=[['Johnson','Matthew']];$0.headers=[['LastName','FirstName']];var $1={};$1.coercionType=Office.CoercionType.Table;Office.select('bindings#'+OfficeApp1Script.AgaveScript.tableBinding).setDataAsync($0,$1);}
...
OfficeApp1Script.AgaveScript.registerClass('OfficeApp1Script.AgaveScript');OfficeApp1Script.AgaveScript.fieldBindingSuffix='FieldBinding';OfficeApp1Script.AgaveScript.rowBindingSuffix='RowBinding';OfficeApp1Script.AgaveScript.tableBinding='TableBinding';OfficeApp1Script.AgaveScript.rowBinding='DataTypes';(function(){Office.initialize=function($p1_0){
$('#Button3').click(OfficeApp1Script.AgaveScript.setInitialData);$('#Button4').click(OfficeApp1Script.AgaveScript.setSecondData);OfficeApp1Script.AgaveScript.setBinding(OfficeApp1Script.AgaveScript.rowBinding,Office.BindingType.Matrix);OfficeApp1Script.AgaveScript.setBinding(OfficeApp1Script.AgaveScript.tableBinding,Office.BindingType.Table);OfficeApp1Script.AgaveScript.populateRowCombo();Office.context.document.addHandlerAsync(Office.EventType.DocumentSelectionChanged,function($p2_0){
$('#eventResults').append('Event fired: '+$p2_0.document.mode.toString()+' Type: '+$p2_0.type.toString()+'<br/>');});};})();
